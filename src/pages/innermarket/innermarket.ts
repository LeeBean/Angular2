...
@Component({
  selector: 'page-innermarket',
  templateUrl: 'innermarket.html'
})
export class InnerMarket {
  pageTitle:string="";
  classIndex=0;
  ...
  constructor(public navCtrl: NavController,private dataApi :DataApi,private tip: Tip,public navParams: NavParams) {
      this.parameter.store_id =dataApi.getStoreId();
      this.parameter.channel_id=navParams.get("channelId");
      this.parameter.page_id=navParams.get("content");
...
//   }
  ...
  goToSlide(item) {
      ...
      if(target_type=="0"){//页面
          this.parameter.page_id=content
